...
| FND-005 | Done | CI job with lint/build/test in `.github/workflows/ci.yml`. | None |
| CAM-001 | Done | Live preview + shutter path in `ios/PhotodewApp/ContentView.swift` and backend capture pipeline. | None |
| CAM-002 | Done | Camera switch control and backend switching implemented. | None |
| CAM-003 | Done | Save-to-Photos flow (including RAW pair save) in `ios/PhotodewApp/PhotodewIOSApp.swift`. | None |
| CAM-004 | Done (accepted) | User-confirmed on-device capture validation for both RAW and processed/JPEG flows; matrix and notes are in `docs/validation/cam-004-orientation-exif-matrix.md`. | None |
| CAM-005 | Done | Capture telemetry now emits `photo_capture_started`, `photo_capture_succeeded`, and `photo_capture_failed` with `capture_latency_ms` in `Sources/CameraKit/CaptureSessionService.swift` and `Tests/CameraKitTests/CaptureSessionServiceTests.swift`. | None |
| CAM-006 | Done (accepted) | Interruption observers + recovery/retry UX copy are implemented in `ios/PhotodewApp/PhotodewIOSApp.swift` and `ios/PhotodewApp/ContentView.swift`; interruption state logging is covered in `Tests/CameraKitTests/CaptureSessionServiceTests.swift`; user validated on device. | None |
| MAN-001 | Done | Exposure state machine scaffolding (`auto`, `locked`, `custom`) is implemented in `Sources/CameraKit/ExposureStateMachine.swift`, integrated into `Sources/CameraKit/CaptureSessionService.swift` and `Sources/App/CaptureAppModel.swift`, with deterministic transitions covered in `Tests/CameraKitTests/ExposureStateMachineTests.swift`. | None |
| MAN-002 | Done (accepted) | Manual ISO/shutter controls are wired from UI to backend in `ios/PhotodewApp/ContentView.swift`, `ios/PhotodewApp/PhotodewIOSApp.swift`, and `Sources/CameraKit/CaptureSessionService.swift`; metadata assertions are covered in `Tests/CameraKitTests/CaptureSessionServiceTests.swift`; on-device validation is documented in `docs/validation/man-002-manual-iso-shutter-validation.md`. | None |
| MAN-003 | Done (accepted) | Manual focus lock state machine and backend apply path are implemented in `Sources/CameraKit/FocusStateMachine.swift` and `Sources/CameraKit/CaptureSessionService.swift`; iOS controls are in `ios/PhotodewApp/ContentView.swift`; deterministic transition coverage is in `Tests/CameraKitTests/FocusStateMachineTests.swift`. | None |
| MAN-004 | Done (accepted) | White balance temperature/tint state machine (`Sources/CameraKit/WhiteBalanceStateMachine.swift`), backend apply/reset in `Sources/CameraKit/CaptureSessionService.swift`, app model passthrough in `Sources/App/CaptureAppModel.swift`, and iOS controls in `ios/PhotodewApp/ContentView.swift` are implemented with tests in `Tests/CameraKitTests/CaptureSessionServiceTests.swift` and `Tests/CameraKitTests/WhiteBalanceStateMachineTests.swift`; user validated on device. | None |
| MAN-005 | Done (accepted) | EV compensation is implemented via backend/service API (`Sources/CameraKit/CaptureSessionService.swift`), app model passthrough (`Sources/App/CaptureAppModel.swift`), and iOS slider/actions (`ios/PhotodewApp/PhotodewIOSApp.swift`, `ios/PhotodewApp/ContentView.swift`) with coverage in `Tests/CameraKitTests/CaptureSessionServiceTests.swift` and `Tests/AppTests/CaptureAppModelTests.swift`; user validated on device. | None |
| MAN-006 | Done (accepted) | Preset slot/domain persistence and default UserDefaults store are implemented in `Sources/App/CaptureControlPreset.swift`; save/apply app-model wiring is in `Sources/App/CaptureAppModel.swift`; iOS preset controls are implemented in `ios/PhotodewApp/PhotodewIOSApp.swift` and `ios/PhotodewApp/ContentView.swift`; coverage is in `Tests/AppTests/CaptureAppModelTests.swift` and `Tests/AppTests/CaptureControlPresetStoreTests.swift`; user validated on device. | None |
| RAW-007 | Done (accepted) | Apple ProRAW capture format (`appleProRAW`) is implemented in `Sources/CameraKit/CaptureSessionService.swift`; mode selection UI and save path are implemented in `ios/PhotodewApp/ContentView.swift` and `ios/PhotodewApp/PhotodewIOSApp.swift`; coverage is in `Tests/CameraKitTests/CaptureSessionServiceTests.swift`; user validated on device. | None |
...
| RAW-005 | Done (accepted) | Metadata pack extraction/persistence is implemented and user-confirmed working on device; implementation in `Sources/CameraKit/CaptureTechnicalMetadata.swift`, `Sources/CameraKit/CaptureSessionService.swift`, `Sources/App/CaptureAppModel.swift`, with tests in `Tests/AppTests/CaptureAppModelTests.swift`. | None |
| RAW-006 | Done (accepted) | Storage warning + cleanup UX is implemented and user-confirmed working on device; implementation in `ios/PhotodewApp/PhotodewIOSApp.swift`, `ios/PhotodewApp/ContentView.swift`, and `Sources/Storage/CaptureMetadataStore.swift`. | None |
| RAW-008 | Done (accepted) | Explicit in-app mode education copy in `ios/PhotodewApp/ContentView.swift` explains True RAW vs Apple ProRAW vs processed tradeoffs; user validated on device. | None |
| UX-001 | Done | Real-time luminance histogram model/analyzer and UI overlay are implemented in `Sources/CameraKit/LuminanceHistogram.swift`, `Sources/CameraKit/CaptureSessionService.swift`, and `ios/PhotodewApp/ContentView.swift`; forwarding and model behavior are covered in `Tests/CameraKitTests/CaptureSessionServiceTests.swift`. | None |
| UX-002 | Done | Zebra clipping thresholding, overlay analyzer, and UI controls are implemented in `Sources/CameraKit/ZebraClippingOverlay.swift`, `Sources/CameraKit/CaptureSessionService.swift`, and `ios/PhotodewApp/ContentView.swift`; coverage is in `Tests/CameraKitTests/CaptureSessionServiceTests.swift`. | None |
| UX-003 | Done (accepted) | Focus peaking overlay model/analyzer and thresholded UI controls are implemented in `Sources/CameraKit/FocusPeakingOverlay.swift`, `Sources/CameraKit/CaptureSessionService.swift`, `Sources/App/CaptureAppModel.swift`, `ios/PhotodewApp/PhotodewIOSApp.swift`, and `ios/PhotodewApp/ContentView.swift`; coverage is in `Tests/CameraKitTests/CaptureSessionServiceTests.swift`; user validated on device. | None |
| UX-004 | Done (accepted) | Horizon level indicator driven by Core Motion device-motion roll is implemented in `ios/PhotodewApp/PhotodewIOSApp.swift` and `ios/PhotodewApp/ContentView.swift`; physical-device validation is recorded in `docs/validation/ux-004-horizon-level-validation.md`. | None |
