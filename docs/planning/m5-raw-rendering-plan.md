...
| Compare interaction | Removed from scope (single-image review only) |
...
Both include:
- profile metadata tags (`kCGImagePropertyProfileName`) and export provenance fields
...
- For processed (JPG) captures: show processed image full-screen
- For RAW/ProRAW captures: render neutral RAW and replace processed preview when ready
...
- Share button → system share sheet with temporary JPEG/TIFF files
- Open in Photos button → opens Photos app
...
3. RAW rendering starts async for RAW/ProRAW captures
4. Once render completes, the display swaps to the rendered RAW image
...
- Store `lastCaptureFormat: CapturePhotoFormat?` to control RAW rendering and export availability
...
| Ticket | Phase | Status |
|---|---|---|
| RND-001 | Phase 1: `RenderKit` module, `RawRenderer`, neutral `CIRAWFilter` settings | Implemented |
| RND-002 | Deferred: Classic and Punch profiles — add as parameterized `RenderingProfile` cases | Deferred |
| RND-003 | Phase 3: `PhotoReviewView` with single-image zoom review and export/share actions | Implemented |
| RND-004 | Phase 2: `RenderExporter` for JPEG and TIFF with color space and profile metadata tags | Implemented |
| RND-005 | Phase 4: `os_signpost` benchmarks, structured logging, <700ms target | In progress |

## Execution Order

```
Phase 1 (RND-001)  →  Phase 2 (RND-004)  →  Phase 3 (RND-003)  →  Phase 4 (RND-005)  →  Phase 5 (wiring)
   done                 done                 done                in progress              done
```

**Total estimate: ~6.5 days (original); remaining: RND-005 hardening**
...
| `ios/PhotodewApp/PhotoReviewView.swift` | Full review screen (single-image mode) |
...
| Share export writes temporary files | Reuse OS temporary directory and allow system cleanup |
